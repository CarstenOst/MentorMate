...
class BookingRepository implements IBookingRepository
{
    ...
    public static function getStudentBookings(DateTime $currentDate, int $studentId): array {
        ...
        $sql = "SELECT Booking.*, User.firstName AS tutorFirstName
            FROM Booking
            JOIN User ON Booking.tutorId = User.userId
            WHERE 
    ...
    public static function getTutorBookings(DateTime $currentDate, int $tutorId): array {
        ...
        $startDate = new DateTime($currentDate->format('Y-m-d') . ' 08:00:00');
        $sql = "SELECT Booking.*, 
        CASE WHEN Booking.studentId IS NOT NULL THEN User.firstName ELSE NULL END AS studentFirstName
        FROM Booking
        ...
        try {
            ...
            if ($results) {
                foreach ($results as $rowKey => $rowValue) {
                    $resultList[] = self::makeBookingFromRow(array_slice($rowValue, 0, 7));
                    $users[] = $rowValue['studentFirstName'];
                }
            }
